def analizar(texto):
    palabras_reservadas = set(['main', 'then', 'if', 'else', 'end', 'do', 'while', 'repeat', 'until', 'cin', 'cout', 'real', 'int', 'integer', 'boolean', 'true', 'false', 'float'])
    simbolos_especiales = {'(': 'PAR_IZQ', ')': 'PAR_DER', '{': 'LLAVE_IZQ', '}': 'LLAVE_DER', ';': 'PUNTO_COMA', ',': 'COMA'}
    operadores_aritmeticos = {'+': 'SUMA', '-': 'RESTA', '*': 'MULTIPLICACION', '/': 'DIVISION', '=': 'ASIGNACION'}
